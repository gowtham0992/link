...
- If the capture inbox looks repetitive, run `python3 link.py dedup-captures .` to preview which captures offer nothing new (already pending, accepted, or dismissed), then re-run with `--confirm` only after the human approves the removal.
